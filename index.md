...
```
    ...
    Markdown 语法：
    ![图片名 #'small/medium/large'](图片地址)

    #small 是一个属性值/attribute value，这需要和开发人员确认属性值的细节才决定在产品信息需要用的属性值。也可以不使用属性值，但例如照片宽度，还有其他格式设置，使用Markdown 是设置不了。
```
